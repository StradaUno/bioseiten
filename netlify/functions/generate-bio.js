...
<body>
  <div class="card">
    \${data.photo_url ? \`<img src="\${data.photo_url}" alt="\${data.name}" class="avatar">\` : \`<div class="avatar-placeholder"></div>\`}
    <h1>\${data.name}</h1>
...
export default async (req) => {
  ...
  try {
    ...
    if (!user_id) return new Response('user_id fehlt', { status: 400 });
    ...
    if (userError || !user) return new Response('User nicht gefunden', { status: 404 });
    if (!user.bio_page_slug) return new Response('Kein Slug gesetzt', { status: 400 });
    ...
    if (!bioPage?.is_active) return new Response('Bio-Page deaktiviert', { status: 400 });
    ...
    await store.set(user.bio_page_slug, html, { metadata: { contentType: 'text/html' } });
    ...
    return new Response(JSON.stringify({ success: true, url: `/u/${user.bio_page_slug}` }), {
      status: 200,
