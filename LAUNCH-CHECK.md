...
8. **Löschen freigeben**: **erledigt am 15.09.2026** (Freigabe Mehmet) — 8 Backup-Tabellen gelöscht (lokale JSON-Sicherung `.launch-check-export-backup-tabellen-20260915.json`, nicht im Git), Buckets `trend-images` (74 Dateien, 147 MB) und `Glenn` (13 Dateien) über die Storage-API geleert und entfernt, Testkonto `launchcheck.test@example.com` samt `users`-Zeile und Vault-Eintrag gelöscht. `legal_texts_backup_20260915` bleibt bis zur Freigabe der Rechtstexte. **Noch offen, nur im Dashboard möglich (MCP kann keine Functions löschen):** 19 Edge Functions löschen — `notify-new-request`, `viuno-config-check`, `viuno-modellvergleich`, `analysis-webhook-befunde-test`, `generate-style-mirror`, `store-news-image`, `resize-news-images`, `viuno-stripe-setup`, `storage-aufraeumen` (Einmal-Werkzeug für die Buckets, jetzt 410) sowie die 10 Stubs `scan-managed-creator`, `fetch-competitor-accounts`, `competitor-webhook`, `apify-webhook`, `fetch-analytics`, `send-collab-reply`, `newsletter-signup`, `get-admin-stats`, `get-news-image`, `sync-news-images`.
...
- Aufräumen (Braucht Mehmet Nr. 8) am 15.09.2026 durchgeführt: Backup-Tabellen, zwei Buckets, Testkonto. Offen bleibt das Löschen der 19 Altlast-Functions im Dashboard.
